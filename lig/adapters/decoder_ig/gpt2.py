...
MLP_ATT_ITB_ZR_A0 = "att_itb_zr_a0"
MLP_ATT_ITB_MAP_A0 = "att_itb_map_a0"
MLP_BASELINES = (
    MLP_ZERO,
    MLP_ITB,
    MLP_ATT_ITB_A0,
...
def normalize_mlp_baseline(name: str) -> str:
    ...
    aliases = {
        ...
        "att_itb_zr_a0": MLP_ATT_ITB_ZR_A0,
        "attitb_zr_a0": MLP_ATT_ITB_ZR_A0,
        "att_itb_map_a0": MLP_ATT_ITB_MAP_A0,
        "attitb_map_a0": MLP_ATT_ITB_MAP_A0,
    }
    if key not in aliases:
...
class GPT2Adapter(DecoderIGAdapter):
    ...
    def decoder(self) -> torch.nn.Module:
        """Inner GPT-2 stack (``transformer`` for LMHeadModel, else ``model``)."""
    ...
    def cache(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        hidden = self.decoder.wte(input_ids) + self.decoder.wpe(position_ids)
